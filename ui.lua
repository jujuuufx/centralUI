...
local themes = {
    preset = {
        accent       = rgb(147, 51, 234),    -- Vibrant purple
        
        background   = rgb(15, 10, 25),      -- Dark purple background
        section      = rgb(25, 15, 40),      -- Slightly lighter purple section
        element      = rgb(35, 20, 55),      -- Element bg with purple tint
        
        outline      = rgb(75, 35, 120),     -- Purple outline
        text         = rgb(255, 255, 255),   -- Clean white
        subtext      = rgb(180, 160, 200),   -- Purple-tinted grey
        
        tab_active   = rgb(45, 25, 75),      -- Active Tab
        tab_inactive = rgb(25, 15, 40),
    },
    utility = {}
}
...
function External:Window(properties)
    ...
    -- Main Window Setup with Enhanced Visual Effects
    ...
    External:Create("UICorner", { Parent = Items.Window, CornerRadius = dim(0, 20) })
    
    -- Add gradient overlay for modern look
    local gradientOverlay = External:Create("Frame", {
        Parent = Items.Window, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 0.9,
        BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = 2
    })
    External:Create("UICorner", { Parent = gradientOverlay, CornerRadius = dim(0, 20) })
    ...
    External:Themify(External:Create("UIStroke", { Parent = Items.Window, Color = themes.preset.outline, Thickness = 2 }), "outline", "Color")


    -- Enhanced Floating Sidebar with Modern Design
    Items.Sidebar = External:Create("Frame", {
        Parent = Items.Window, Position = dim2(0, 12, 0, 12), Size = dim2(0, 80, 1, -24),
        BackgroundColor3 = themes.preset.section, BorderSizePixel = 0, ZIndex = 3
    })
    External:Themify(Items.Sidebar, "section", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.Sidebar, CornerRadius = dim(0, 16) })
    
    -- Add subtle gradient to sidebar
    local sidebarGradient = External:Create("Frame", {
        Parent = Items.Sidebar, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 0.7,
        BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = 4
    })
    External:Create("UICorner", { Parent = sidebarGradient, CornerRadius = dim(0, 16) })
    External:Create("UIGradient", { 
    ...
    External:Themify(External:Create("UIStroke", { Parent = Items.Sidebar, Color = themes.preset.outline, Thickness = 1.5 }), "outline", "Color")
    ...
    -- Enhanced Floating Header with Modern Design
    Items.Header = External:Create("Frame", { 
        Parent = Items.Window, Position = dim2(0, 102, 0, 12), Size = dim2(1, -114, 0, 70), 
        BackgroundColor3 = themes.preset.section, Active = true, ZIndex = 3 
    })
    External:Themify(Items.Header, "section", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.Header, CornerRadius = dim(0, 16) })
    
    -- Add header gradient effect
    local headerGradient = External:Create("Frame", {
        Parent = Items.Header, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 0.8,
        BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = 4
    })
    External:Create("UICorner", { Parent = headerGradient, CornerRadius = dim(0, 16) })
    External:Create("UIGradient", { 
        Parent = headerGradient, 
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, themes.preset.accent),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(120, 40, 200)),
            ColorSequenceKeypoint.new(1, themes.preset.section)
        }),
        Rotation = 0,
        Transparency = NumberSequence.new({
    ...
    External:Themify(External:Create("UIStroke", { Parent = Items.Header, Color = themes.preset.outline, Thickness = 1.5 }), "outline", "Color")
    ...
    Items.LogoText = External:Create("TextLabel", {
        ...
        BackgroundTransparency = 1, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Bold), TextSize = 18, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 4
    })
    ...
    Items.SubLogoText = External:Create("TextLabel", {
        ...
        Size = dim2(0, 0, 0, 18), AutomaticSize = Enum.AutomaticSize.X,
        BackgroundTransparency = 1, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.SemiBold), TextSize = 18, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 4
    })
    ...
    Items.AvatarFrame = External:Create("Frame", {
        ...
        Size = dim2(0, 36, 0, 36), BackgroundColor3 = themes.preset.element, BorderSizePixel = 0, ZIndex = 5
    })
    ...
    Items.Username = External:Create("TextLabel", {
        ...
        AnchorPoint = vec2(1, 0.5), Position = dim2(1, -68, 0.5, 0), Size = dim2(0, 150, 0, 16),
        BackgroundTransparency = 1, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.SemiBold), TextSize = 15, TextXAlignment = Enum.TextXAlignment.Right, ZIndex = 5
    })
    External:Themify(Items.Username, "text", "TextColor3")

    Items.SettingsBtn = External:Create("ImageButton", {
        Parent = Items.Header, AnchorPoint = vec2(1, 0.5), Position = dim2(1, -235, 0.5, 0),
        Size = dim2(0, 20, 0, 20), BackgroundTransparency = 1, Image = "rbxassetid://10734950309", ImageColor3 = themes.preset.subtext, ZIndex = 5
    })
    ...
    Items.Username:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
        Items.SettingsBtn.Position = dim2(1, -95 - Items.Username.TextBounds.X, 0.5, 0)
    end)
    Items.SettingsBtn.Position = dim2(1, -95 - Items.Username.TextBounds.X, 0.5, 0)
    ...
    Items.PageHolder = External:Create("Frame", { 
        Parent = Items.Window, Position = dim2(0, 102, 0, 94), Size = dim2(1, -114, 1, -106), 
...
function External:Tab(properties)
    ...
    function Cfg.OpenTab()
        ...
        if Items.Button then 
            ...
            External:Tween(Items.IconImg, {ImageColor3 = rgb(255,255,255)}, buttonTween) -- Pops white against the accent background
        end
...
function External:Section(properties)
    ...
    External:Create("UICorner", { Parent = Items.Section, CornerRadius = dim(0, 12) })
    ...
    Items.Dot = External:Create("Frame", { 
        Parent = Items.Header, Position = dim2(0, 16, 0.5, 0), AnchorPoint = vec2(0, 0.5), Size = dim2(0, 8, 0, 8), 
    ...
    Items.Title = External:Create("TextLabel", { 
        Parent = Items.Header, Position = dim2(0, 32, 0.5, 0), AnchorPoint = vec2(0, 0.5), Size = dim2(1, -48, 0, 16), 
        BackgroundTransparency = 1, Text = Cfg.Name, TextColor3 = themes.preset.text, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Bold), TextSize = 15, TextXAlignment = Enum.TextXAlignment.Left 
    })
...
function External:ScriptCard(properties)
    ...
    Items.Container = External:Create("Frame", { 
        Parent = self.Items.Container, Size = dim2(1, 0, 0, 65), BackgroundColor3 = themes.preset.element, 
        BorderSizePixel = 0 
    })
    External:Themify(Items.Container, "element", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.Container, CornerRadius = dim(0, 12) })
    
    -- Add subtle gradient overlay to script cards
    local cardGradient = External:Create("Frame", {
        Parent = Items.Container, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 0.85,
        BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = 1
    })
    External:Create("UICorner", { Parent = cardGradient, CornerRadius = dim(0, 12) })
    External:Create("UIGradient", { 
        Parent = cardGradient, 
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, themes.preset.accent),
    ...
    External:Themify(External:Create("UIStroke", { Parent = Items.Container, Color = themes.preset.outline, Thickness = 1.5 }), "outline", "Color")

    Items.IconFrame = External:Create("Frame", {
        Parent = Items.Container, Position = dim2(0, 14, 0.5, 0), AnchorPoint = vec2(0, 0.5),
        Size = dim2(0, 42, 0, 42), BackgroundColor3 = themes.preset.background, BorderSizePixel = 0, ZIndex = 2
    })
    External:Themify(Items.IconFrame, "background", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.IconFrame, CornerRadius = dim(0, 10) })
    External:Themify(External:Create("UIStroke", { Parent = Items.IconFrame, Color = themes.preset.accent, Thickness = 2 }), "accent", "Color")

    Items.Icon = External:Create("ImageLabel", {
        Parent = Items.IconFrame, Position = dim2(0.5, 0, 0.5, 0), AnchorPoint = vec2(0.5, 0.5),
        Size = dim2(0, 22, 0, 22), BackgroundTransparency = 1, Image = Cfg.Icon, ImageColor3 = themes.preset.accent, ZIndex = 3
    })
    External:Themify(Items.Icon, "accent", "ImageColor3")

    Items.Title = External:Create("TextLabel", {
        Parent = Items.Container, Position = dim2(0, 70, 0, 14), Size = dim2(1, -150, 0, 18), ZIndex = 2,
        BackgroundTransparency = 1, Text = Cfg.Name, TextColor3 = themes.preset.text,
        FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Bold), TextSize = 16, TextXAlignment = Enum.TextXAlignment.Left
    })
    External:Themify(Items.Title, "text", "TextColor3")

    Items.Desc = External:Create("TextLabel", {
        Parent = Items.Container, Position = dim2(0, 70, 0, 36), Size = dim2(1, -150, 0, 16), ZIndex = 2,
        BackgroundTransparency = 1, Text = Cfg.Description, TextColor3 = themes.preset.subtext,
        FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium), TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left
    })
    External:Themify(Items.Desc, "subtext", "TextColor3")

    Items.LoadBtn = External:Create("TextButton", {
        Parent = Items.Container, AnchorPoint = vec2(1, 0.5), Position = dim2(1, -14, 0.5, 0), ZIndex = 2,
        Size = dim2(0, 70, 0, 35), BackgroundColor3 = themes.preset.accent, Text = "RUN",
        TextColor3 = rgb(255, 255, 255), FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Bold), TextSize = 13, AutoButtonColor = false
    })
    External:Themify(Items.LoadBtn, "accent", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.LoadBtn, CornerRadius = dim(0, 8) })
    
    -- Add hover effect gradient to button
    local btnGradient = External:Create("Frame", {
        Parent = Items.LoadBtn, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 0.8,
        BackgroundColor3 = rgb(255, 255, 255), BorderSizePixel = 0, ZIndex = 3
    })
    External:Create("UICorner", { Parent = btnGradient, CornerRadius = dim(0, 8) })
    External:Create("UIGradient", { 
        Parent = btnGradient, 
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, rgb(255, 255, 255)),
            ColorSequenceKeypoint.new(1, rgb(200, 200, 255))
        }),
        Rotation = 45,
        Transparency = NumberSequence.new({
    ...
    Items.LoadBtn.MouseButton1Click:Connect(function()
        External:Tween(Items.LoadBtn, {BackgroundColorTransparency = 0.3, Size = dim2(0, 65, 0, 30)}, TweenInfo.new(0.1))
        task.wait(0.1)
        External:Tween(Items.LoadBtn, {BackgroundColorTransparency = 0, Size = dim2(0, 70, 0, 35)}, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
        Cfg.Callback()
    end)
...
function External:Toggle(properties)
    ...
    Items.Button = External:Create("TextButton", { 
        Parent = self.Items.Container, Size = dim2(1, 0, 0, 45), 
        BackgroundColor3 = themes.preset.element, Text = "", AutoButtonColor = false
    })
    External:Themify(Items.Button, "element", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.Button, CornerRadius = dim(0, 10) })
    
    -- Add subtle gradient to toggle background
    local toggleGradient = External:Create("Frame", {
        Parent = Items.Button, Size = dim2(1, 0, 1, 0), BackgroundTransparency = 0.9,
        BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = 1
    })
    External:Create("UICorner", { Parent = toggleGradient, CornerRadius = dim(0, 10) })
    External:Create("UIGradient", { 
        Parent = toggleGradient, 
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, themes.preset.accent),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(100, 30, 180))
        }),
    ...
    External:Themify(External:Create("UIStroke", { Parent = Items.Button, Color = themes.preset.outline, Thickness = 1.5 }), "outline", "Color")

    -- Enhanced iOS Style Switch Pill
    Items.SwitchBG = External:Create("Frame", { 
        Parent = Items.Button, AnchorPoint = vec2(1, 0.5), Position = dim2(1, -14, 0.5, 0), Size = dim2(0, 44, 0, 24), 
    ...
    External:Themify(External:Create("UIStroke", { Parent = Items.SwitchBG, Color = themes.preset.outline, Thickness = 1.5 }), "outline", "Color")

    Items.SwitchKnob = External:Create("Frame", {
        Parent = Items.SwitchBG, AnchorPoint = vec2(0, 0.5), Position = dim2(0, 4, 0.5, 0), Size = dim2(0, 18, 0, 18),
    ...
    -- Add subtle glow effect to knob
    local knobGlow = External:Create("Frame", {
        Parent = Items.SwitchKnob, Size = dim2(1, 4, 1, 4), Position = dim2(0, -2, 0, -2),
        BackgroundColor3 = themes.preset.accent, BorderSizePixel = 0, ZIndex = -1
    })
    External:Create("UICorner", { Parent = knobGlow, CornerRadius = dim(1, 0) })
    External:Create("UIGradient", { 
        Parent = knobGlow, 
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, themes.preset.accent),
    ...
    Items.Title = External:Create("TextLabel", { 
        Parent = Items.Button, Position = dim2(0, 16, 0.5, 0), AnchorPoint = vec2(0, 0.5), Size = dim2(1, -72, 1, 0), 
        BackgroundTransparency = 1, Text = Cfg.Name, TextColor3 = themes.preset.subtext, TextSize = 14, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium), TextXAlignment = Enum.TextXAlignment.Left 
    })
    ...
    function Cfg.set(bool)
        State = bool
        External:Tween(Items.SwitchBG, {BackgroundColor3 = State and themes.preset.accent or themes.preset.background}, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out))
        External:Tween(Items.SwitchKnob, {Position = State and dim2(0, 21, 0.5, 0) or dim2(0, 3, 0.5, 0), BackgroundColor3 = State and rgb(255,255,255) or themes.preset.subtext}, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
        External:Tween(Items.Title, {TextColor3 = State and themes.preset.text or themes.preset.subtext}, TweenInfo.new(0.2))
        
        -- Animate glow effect
        if State then
            External:Tween(knobGlow, {Size = dim2(1.2, 0, 1.2, 0), Position = dim2(-0.1, 0, -0.1, 0)}, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
        else
            External:Tween(knobGlow, {Size = dim2(1, 4, 1, 4), Position = dim2(0, -2, 0, -2)}, TweenInfo.new(0.2))
        end
...
function External:Button(properties)
    ...
    Items.Button = External:Create("TextButton", { 
        Parent = self.Items.Container, Size = dim2(1, 0, 0, 40), BackgroundColor3 = themes.preset.element, 
        Text = Cfg.Name, TextColor3 = themes.preset.text, TextSize = 14, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.SemiBold), AutoButtonColor = false 
    })
    External:Themify(Items.Button, "element", "BackgroundColor3")
    External:Themify(Items.Button, "text", "TextColor3")
    External:Create("UICorner", { Parent = Items.Button, CornerRadius = dim(0, 8) })
    ...
    Items.Button.MouseButton1Click:Connect(function()
        External:Tween(Items.Button, {Size = dim2(0.98, 0, 0, 34), BackgroundColor3 = themes.preset.accent}, TweenInfo.new(0.1))
        task.wait(0.1)
        External:Tween(Items.Button, {Size = dim2(1, 0, 0, 36), BackgroundColor3 = themes.preset.element}, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out))
        Cfg.Callback()
    end)
    return setmetatable(Cfg, External)
end
...
function External:Slider(properties)
    ...
    Items.ContainerBox = External:Create("Frame", { 
        Parent = self.Items.Container, Size = dim2(1, 0, 0, 56), 
        BackgroundColor3 = themes.preset.element 
    })
    External:Themify(Items.ContainerBox, "element", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.ContainerBox, CornerRadius = dim(0, 8) })
    ...
    Items.Title = External:Create("TextLabel", { Parent = Items.ContainerBox, Position = dim2(0, 16, 0, 8), Size = dim2(1, -32, 0, 20), BackgroundTransparency = 1, Text = Cfg.Name, TextColor3 = themes.preset.text, TextSize = 14, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Medium), TextXAlignment = Enum.TextXAlignment.Left })
    External:Themify(Items.Title, "text", "TextColor3")

    Items.Val = External:Create("TextLabel", { Parent = Items.ContainerBox, Position = dim2(0, 16, 0, 8), Size = dim2(1, -32, 0, 20), BackgroundTransparency = 1, Text = tostring(Cfg.Default)..Cfg.Suffix, TextColor3 = themes.preset.accent, TextSize = 14, FontFace = Font.new("rbxassetid://12187365364", Enum.FontWeight.Bold), TextXAlignment = Enum.TextXAlignment.Right })
    External:Themify(Items.Val, "accent", "TextColor3")

    -- [CREATIVE SWITCH]: Thicker modern slider track
    Items.Track = External:Create("TextButton", { Parent = Items.ContainerBox, Position = dim2(0, 16, 0, 36), Size = dim2(1, -32, 0, 10), BackgroundColor3 = themes.preset.background, Text = "", AutoButtonColor = false })
    ...
    -- Knob
    Items.Knob = External:Create("Frame", {Parent = Items.Fill, AnchorPoint = vec2(1, 0.5), Position = dim2(1, 5, 0.5, 0), Size = dim2(0, 16, 0, 16), BackgroundColor3 = rgb(255,255,255)})
...
function External:Textbox(properties)
    ...
    Items.ContainerBox = External:Create("Frame", { 
        Parent = self.Items.Container, Size = dim2(1, 0, 0, 40), 
        BackgroundColor3 = themes.preset.element 
    })
    External:Themify(Items.ContainerBox, "element", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.ContainerBox, CornerRadius = dim(0, 8) })
...
function External:Label(properties)
    ...
    Items.ContainerBox = External:Create("Frame", { 
        Parent = self.Items.Container, Size = dim2(1, 0, 0, Cfg.Wrapped and 44 or 40), 
        BackgroundColor3 = themes.preset.element 
    })
    External:Themify(Items.ContainerBox, "element", "BackgroundColor3")
    External:Create("UICorner", { Parent = Items.ContainerBox, CornerRadius = dim(0, 8) })
...
function Notifications:Create(properties)
    ...
    External:Create("UIPadding", { Parent = Items.Name; PaddingTop = dim(0, 12); PaddingBottom = dim(0, 12); PaddingRight = dim(0, 14); PaddingLeft = dim(0, 14); })
